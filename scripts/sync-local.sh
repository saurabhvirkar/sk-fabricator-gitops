...
if [ -d "${WORKSPACE_ROOT}/SkFabricatorAndErector-Backend" ]; then
  echo "Building saurabhvirkar/skfabricator-backend:dev-latest..."
  docker build -t saurabhvirkar/skfabricator-backend:dev-latest "${WORKSPACE_ROOT}/SkFabricatorAndErector-Backend"
fi

if [ -d "${WORKSPACE_ROOT}/SkFabricatorAndErector-Frontend" ]; then
  echo "Building saurabhvirkar/skfabricator-frontend:dev-latest..."
  docker build -t saurabhvirkar/skfabricator-frontend:dev-latest "${WORKSPACE_ROOT}/SkFabricatorAndErector-Frontend"
fi
...
if command -v kind &> /dev/null && kind get clusters 2>/dev/null | grep -q "^skops$"; then
  kind load docker-image saurabhvirkar/skfabricator-backend:dev-latest --name skops
  kind load docker-image saurabhvirkar/skfabricator-frontend:dev-latest --name skops
fi
